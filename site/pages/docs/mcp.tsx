...
`}</Screen>
			...
			<section>
				...
				<p>
					...
					<Link href="/docs/grants/">the egress proxy</Link>, a remote one is reached down that same
					road, and the proxy already writes whatever key either of them needs. So connecting to a
					server that wants an account is still two things: the line, and a way in. Which way is not
					a question the operator should have to answer out of a README, so the server is asked —{" "}
					<code>initialize</code> is what any client sends first, and a server that would refuse the
					agent refuses that identically.
				</p>
				<Screen>{`
